...
class ChaseParser(BaseBankParser):
    ...
    def _detect_section(self, line: str) -> Optional[str]:
        ...
        if any(pattern in line_lower for pattern in [
            "retiros electrónicos", "electronic withdrawals", "detalle de transacciones"
        ]):
            return "withdrawals"
    ...
    def _clean_description(self, text: str) -> str:
        ...
        cleaned = re.sub(r"\s*cantidad\s+saldo\s*$", "", cleaned, flags=re.I)
    ...
    def _determine_direction(self, description: str, section_context: str, amount: float, full_text: str) -> Optional[str]:
        ...
        # Spanish ACH debit patterns (débito = debit = out)
        if any(pattern in desc_lower for pattern in [
            "débito de cámara", "débito automatizado", "wise"
        ]):
            return "out"
